...
def calcLen(seq, Vocab):
    lengths = [len(Vocab['i2w'][token])+1 for token in seq]
    return sum(lengths) - 1

def bfs_beam_75_search(encoder, encoderInputs, decoder, otherInputs, Vocab, options, log):
    ...
    for step in range(0, maxStep):
        ...
        for i in range(0, len(bestNextStates)):
            if (bestNextStates[i][1][-1] == 1):
                if (calcLen(bestNextStates[i][1], Vocab) >= 87):
                    n_top += 1
    ...
    while (len(lastCandidates) > 0) and (n_top < options['beam_size']):
        #print n_top
        n_top += 1
